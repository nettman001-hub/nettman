...
저장한 완성 설교는 `/study`와 `/ministry`에서 후속 자료로 확장할 수 있습니다. 스터디와 사역 활용은 기본/고급/고급추론 1/2/4토큰을 차감하며, 계정당 하루 20회·동시 1건으로 제한됩니다. 스터디·사역 활용·설교 비평·설교도우미 코치·본문 확인은 탭 전역 AI 실행기로 관리되어 다른 업무 메뉴로 이동해도 계속되며, 원래 화면과 상단 상태 표시에서 언제든 중지할 수 있습니다.
...
로그인 사용자는 가입 시 받은 200토큰에서 설교 생성 비용을 사용합니다. 10분·1포인트 기준 기본/고급/고급추론은 약 10/20/40토큰부터이며, 실제 비용은 AI 엔진, 설교 분량과 대지 수로 계산하고 초안 개수와는 무관하게 생성 작업 ID당 한 번만 차감합니다. 스터디와 사역 활용은 생성 1회당 기본/고급/고급추론 1/2/4토큰입니다. 충전 결제 연결 여부와 토큰 사용 원장은 서로 분리되어 있습니다.
