...
export function ConsultationRoom({ id, expertMode = false }: { id: string; expertMode?: boolean }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!response.ok || !payload.item) throw new Error(payload.error || "피드백 요청을 찾을 수 없습니다.");
        setItem(payload.item);
        setMessages(Array.isArray(payload.messages) ? payload.messages : []);
        setState("ready");
      } catch (error) {
        if (error instanceof DOMException && error.name === "AbortError") return;
        setNotice(error instanceof Error ? error.message : "피드백을 불러오지 못했습니다.");
        setState("error");
      }
    }
    void load();
  ...
  async function complete() {
    if (!item || !window.confirm("이 피드백을 완료 상태로 바꿀까요? 완료 후에는 메시지를 더 보낼 수 없습니다.")) return;
    setState("sending");
    try {
      const response = await fetch(`/api/consultations/${encodeURIComponent(id)}`, {
        method: "POST",
  ...
  if (!item) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-16 text-center">
        <div className="rounded-[2rem] border border-[#e2b8ae] bg-white p-9"><h1 className="font-serif text-2xl font-bold text-[#294238]">피드백을 열 수 없습니다</h1><p className="mt-3 text-sm text-[#7b4a42]">{notice}</p><a href={expertMode ? "/expert" : "/consult"} className="mt-6 inline-flex min-h-11 items-center rounded-xl bg-[#285343] px-5 text-sm font-bold text-white">목록으로 돌아가기</a></div>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-7 sm:px-8 lg:px-12 lg:py-10">
      <div className="flex flex-col gap-5 border-b border-[#d8d2c7] pb-6 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <a href={expertMode ? "/expert" : "/consult"} className="text-xs font-extrabold text-[#8d5a2e] hover:underline">← {expertMode ? "설교 피드백 목록" : "피드백 목록"}</a>
        ...
        <div className="flex gap-2">
          ...
          {expertMode && item.status !== "completed" ? <button type="button" onClick={() => void complete()} disabled={state === "sending"} className="min-h-11 rounded-xl bg-[#285343] px-4 text-xs font-extrabold text-white disabled:opacity-60">피드백 완료</button> : null}
        </div>
      </div>

      <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_18rem]">
        <section className="overflow-hidden rounded-[1.75rem] border border-[#ddd7cd] bg-white shadow-[0_16px_45px_rgba(39,50,44,.06)]" aria-label="피드백 대화">
          <div className="max-h-[34rem] min-h-[28rem] overflow-y-auto px-4 py-6 sm:px-7">
            {messages.length === 0 ? <div className="grid min-h-[22rem] place-items-center text-center"><div><span aria-hidden="true" className="mx-auto grid size-12 place-items-center rounded-2xl bg-[#e7eee9] font-serif font-bold text-[#315746]">피</span><p className="mt-4 text-sm font-extrabold text-[#34463e]">첫 답변을 기다리고 있습니다</p><p className="mt-1 text-xs leading-5 text-[#7a837e]">전문가가 배정되면 이 대화에서 의견을 나눌 수 있습니다.</p></div></div> : <ol className="space-y-5">{messages.map((message) => {
        ...
        <aside className="space-y-4">
          ...
          <section className="rounded-[1.4rem] border border-[#ddd7cd] bg-white p-5"><h2 className="text-sm font-extrabold text-[#334b40]">좋은 피드백을 위한 안내</h2><ul className="mt-3 space-y-2 text-xs leading-5 text-[#747d78]"><li>• 특정 문단을 말할 때 관련 부분을 선택해 주세요.</li><li>• 제안은 원고에 자동 반영되지 않습니다.</li><li>• 성경 해석과 최종 전달 책임은 설교자에게 있습니다.</li></ul></section>
        </aside>
      </div>
    </div>
  );
