...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!input.options || typeof input.options.topic !== "string") {
  ...
  const db = getD1();
  const userAi = await getManagedAiRequestConfig(db, input.options.aiTier);
  if (input.options.aiTier !== "basic" && !userAi) {
