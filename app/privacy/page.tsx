...
const sections: readonly LegalSection[] = [
    ...
    content: (
      <>
        ...
        <p>
          ...
          단계별 설교도우미, 설교 피드백, 스터디·사역 활용, 알림 설정, 관리자 AI 엔진 설정 및 토큰 충전 기능을 기준으로 합니다.
        </p>
      </>
    ),
  },
    ...
    content: (
      <>
        ...
        <ul>
          ...
          <li>계정 이용 상태, 정지 사유·기한, 최근 접속 시각과 서버가 확인한 세션 식별자·폐기 시각</li>
        </ul>
        ...
        <ul>
          ...
          <li>
            설교도우미 작업의 제목·본문, 현재 단계, 단계별 메모와 입력 항목, 직접 작성한
            원고, 완료 여부와 수정 시각
          </li>
          <li>
            이용자가 기록한 참고 출처와 확인 여부, 명시적으로 채택한 AI 코치 제안 및
            해당 제안이 사용된 단계
          </li>
          <li>
            AI 코치 요청 식별자·처리 상태·차감 토큰과 중복 요청 재생을 위해 일시 보관하는
            AI 코치 응답
          </li>
        </ul>
        ...
        <h3>관리자 회원 지원과 감사</h3>
        <ul>
          <li>관리자가 수행한 역할·이용 상태·무료 토큰·인증 지원·결제 재검증 작업과 사유</li>
          <li>작업 관리자와 대상 회원 식별자, 고유 요청 번호, 변경 전후 값과 처리 시각</li>
          <li>회원 지원과 부정 이용 방지를 위해 권한 있는 관리자가 프로필, 활동·원장 메타데이터와 인증 상태를 확인할 수 있습니다.</li>
          <li>회원관리 기본 화면에서는 저장 설교 본문과 설교 피드백 메시지를 표시하지 않습니다.</li>
        </ul>
      </>
    ),
  },
    ...
    content: (
      <>
        <p>
          ...
          설정, 설교·설교도우미 작업과 피드백 기록은 서비스에 연결된 <strong>Supabase PostgreSQL</strong>
          데이터베이스에 저장될 수 있습니다.
        </p>
        ...
        <ul>
          ...
          <li>
            <strong>sessionStorage:</strong> 네트워크 중단 뒤 중복 차감 없이 동일한 AI 코치
            요청을 다시 확인하기 위해, 현재 단계의 메모·선택한 원고 발췌·질문과 요청 식별자를
            현재 탭에 최대 24시간 임시 보관합니다. 성공·명시적 환불·만료·충돌이 확인되면
            삭제하며, 그전에는 입력을 잠가 다른 내용으로 바뀌지 않게 합니다.
          </li>
        </ul>
        <p>
          AI 공급자 API 키 원문은 브라우저에 저장하거나 다시 표시하지 않습니다. 관리자가
          엔진 설정에서 등록하면 암호화해 데이터베이스에 저장하고, 저장된 키가 없을 때는
          서버의 비밀 환경 변수를 사용할 수 있습니다. 일반 이용자는 엔진, 모델, API 주소
          또는 키를 변경할 수 없습니다.
        </p>
        <p>
          ...
          AI 코치 재시도 정보도 같은 탭을 계속 사용하는 사람이 볼 수 있으므로 공유 기기에서는
          탭을 닫거나 사이트 데이터를 삭제해 주세요.
        </p>
      </>
    ),
  },
      ...
      <>
        ...
        <ul>
          ...
          <li>
            ...
            조건·본문·참고 자료·수정 지시, 스터디·사역 활용 요청과 필요한 신학·사역 문맥을 전송합니다.
            설교도우미 AI 코치를 이용하면 현재 단계에서 이용자가 작성한 내용, 질문 및
            선택한 참고 출처의 필요한 부분이 전송될 수 있습니다.
          </li>
          <li>
  ...
  },
  {
    ...
    content: (
      <>
        <p>
          계정, 프로필, 설정, 토큰·결제 원장, 서버에 저장한 설교·설교도우미 작업과 피드백
          기록에는 아래 AI 코치 응답 예외 외에는 현재 별도의 자동 삭제 기한이 설정되어 있지
          않습니다. 브라우저 localStorage의 정보도 이용자가 브라우저 데이터를 지우기 전까지
          남을 수 있습니다.
        </p>
        <ul>
          <li>
            진행 중인 설교도우미 작업에서 &quot;이 준비 삭제&quot;를 선택하면 제목, 성경 본문,
            단계별 입력과 원고, 출처 내용을 즉시 제거합니다. 중복 처리와 삭제 상태 확인을 위해
            작업 식별자, 계정 식별자, 생성·수정·삭제 시각만 담은 최소 삭제 표시는 남을 수 있습니다.
          </li>
          ...
          <li>
            중복 전송 때 같은 결과를 안전하게 재생하는 기간은 AI 코치 응답 생성 후
            24시간입니다. 기간이 지나면 결과는 더 이상 재생하지 않으며, 다음 AI 코치 이용
            또는 토큰 지갑 조회 때 응답 내용을 자동 삭제합니다. 과금·환불과 중복 실행
            방지에 필요한 최소 요청 식별자와 처리 상태는 토큰 원장 확인을 위해 계속 보관될
            수 있습니다.
          </li>
        </ul>
        <p>
    ...
    content: (
      <>
        <p>
          ...
          암호화해 데이터베이스에 저장하거나 서버 비밀 변수로 분리합니다.
        </p>
        <p>
          관리자 회원 작업은 사유와 고유 요청 번호를 기록하고, 이용 정지와 폐기된 세션을
          서버에서 중앙 확인합니다. 인증 관리자 키는 서버에서만 사용하며 브라우저에 전달하지 않습니다.
        </p>
        <p>
