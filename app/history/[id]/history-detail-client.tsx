    ...
    <div className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-7 sm:py-10 lg:px-10 lg:py-12">
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_17rem] lg:items-start">
        <article className="rounded-3xl border border-[#d8d2c7] bg-[#fffdf8] px-6 py-9 shadow-[0_18px_55px_rgba(31,54,45,.06)] sm:px-10 lg:px-14 lg:py-14">
          <div className="flex flex-wrap items-center gap-2 text-xs font-bold text-[#7c8882]"><span className="rounded-full bg-[#e4ece3] px-3 py-1.5 text-[#41634e]">{sermon.sermonType}</span><span>{sermon.audience}</span><span>·</span><span>{sermon.duration}분</span><span>·</span><span>{sermon.pointCount}대지</span><span>·</span><span>{sermon.emotion}</span></div>
...
function toOptions(sermon: SermonRecord): SermonOptions {
  return {
    ...
    duration: [5, 10, 20, 30].includes(sermon.duration) ? sermon.duration as SermonOptions["duration"] : 20,
