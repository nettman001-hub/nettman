...
```text
/sermon-helper              목회자 주도 8단계 준비·자동 저장·선택형 AI 코치
...
### 목회자 주도 설교도우미 (2026-08-21)

- 좌측 업무 메뉴에서 `홈 → 설교도우미 → 새 설교` 순으로 노출합니다. 홈과 공개 랜딩의 1차 경로는 `/sermon-helper`, 기존 다섯 AI 초안 생성은 `/sermon/options`의 별도 선택지로 유지합니다.
- 설교도우미는 `상황 → 본문 → 연구 → 한 문장 메시지 → 구조 → 적용 → 직접쓰기 → 최종 점검`의 8단계를 네 흐름으로 보여 줍니다. 단계는 잠그지 않고 자유롭게 이동하며 서버 자동 저장, 버전·수정 시각 CAS 충돌 검사와 저장 전 이동 보존을 적용합니다.
- 작업은 기존 `sermon_drafts`와 분리한 `sermon_helper_projects`에 저장합니다. 단계별 목회자 메모, 직접 작성 원고, 확인한 성경·외부 자료와 명시적으로 채택한 AI 제안을 provenance로 구분합니다.
- 성경 본문은 자유 형식 입력을 기존 AI 본문 정규화 경로로 확인하며, AI 결과가 입력과 같아도 목회자가 `직접 확인했고 이 표기 적용`을 눌러야 확인 출처로 기록합니다. 실제 성경 전문을 모델 기억으로 만들어 표시하지 않습니다.
- AI 코치는 생각 질문·연구 방향·점검·부분 다듬기의 현재 한 단계만 다룹니다. 전체 원고 자동 작성은 계약과 프롬프트에서 금지하고, 응답은 `내 작업에 채택` 전에는 저장하지 않습니다. 기본/고급/고급 추론은 요청당 1/2/4토큰이며 동일 메시지 중복 차단과 실패·중단 자동 환불을 적용합니다.
- 완료는 제목·본문, 도입·결론·적용, 제목과 내용이 있는 1~4개 대지, 직접쓰기·점검 완료 및 여섯 체크 항목을 서버가 다시 검사합니다. 저장 설교 목록·상세에는 `목회자 작성 · AI 보조`, 기존 자동 초안에는 `AI 초안 기반` 배지를 표시합니다.
- 관련 마이그레이션은 `drizzle/0016_strong_thunderball.sql`부터 이어지며, 사용자별 소유권·RLS, 전체 프로젝트 크기·생성 쿼터, 완료 트랜잭션과 AI 코치 요청 수명주기를 배포 전 회귀 테스트로 검증합니다.
...
- `/study`, `/critique`, `/ministry`를 추가했습니다. 스터디는 직접 입력한 본문, 비평은 직접 입력한 원고, 사역 활용은 저장한 완성 설교를 사용합니다. 스터디·사역 활용은 기본/고급/고급추론 1/2/4토큰을 차감하고, 세 기능 모두 계정당 하루 20회·동시 1건의 공정 이용 한도를 적용합니다.
- 스터디·사역 활용·설교 비평·설교도우미 코치·본문 확인 요청은 `background-ai-runner.ts`가 탭 전역에서 소유합니다. SPA 메뉴 이동으로 요청이 중단되지 않으며 원래 화면과 AppShell 상단에서 중지할 수 있습니다.
...
- (2026-08-20) 설교 깊이 파이프라인: 모든 호스팅 초안은 번들된 개역한글(1961) 본문·TSK 교차참조를 주입받고(`app/_lib/bible/`), 위치 1에서 다섯 초안 공용의 설계 계약(Big Idea·FCF·절-대지 매핑·그리스도 연결)을 1회 생성해 `sermon_generation_parts (position 0, step 0)`에 저장한 뒤 나머지 위치가 재사용합니다. 고급 추론 등급은 추가로 공유 주해 브리프(position 0, step 1)와 초안별 깊이 판정(step 90, 기본 등급 엔진으로 채점) 후 high 심각도 발견 1건에 한해 해당 섹션만 내부 수정 호출로 보완합니다(사용자 수정 3회와 무관, 과금 불변). 모든 단계 실패는 해당 단계 생략으로 우아하게 강등되며 `[sermon-ai] deep-pipeline` 콘솔 로그와 parts의 elapsed_ms가 원가 계측 근거입니다.
...
10분·1포인트는 약 10/20/40토큰부터, 30분·4대지는 36/72/144토큰입니다. 같은 생성 ID의 초안 1~5와 조각 생성·재개는 중복 차감하지 않습니다.
...
- 각 엔진의 `최대 출력 토큰`은 선택값입니다. 공란(`NULL`)이면 설교 분량·작업 단계별 자동값을 사용하고, 숫자를 저장하면 해당 엔진의 설교 원고 생성·수정(장문) 요청에만 적용합니다. 개요·조각·판정 같은 짧은 보조 호출은 자동값을 유지해 설정 하나가 다단계 생성 전체를 파손하지 못하게 막습니다. 성경 본문 판정은 기존 500토큰 한도를 유지합니다. Anthropic 엔진에서 추론 강도를 사용하면 사고 토큰이 원고 예산을 잠식하지 않도록 max_tokens에 8,000토큰의 헤드룸을 가산합니다.
...
| 설교도우미 계약·UI | `app/_lib/sermon-helper-types.ts`, `app/sermon-helper` | 8단계 목회자 작업, 출처·제안 채택, 자동 저장, 완료 검증 |
| 설교도우미 API·AI 코치 | `app/api/sermon-helper`, `app/_lib/sermon-helper-*.ts` | 소유권 CRUD, 완료 저장, 4모드 코치, 1/2/4토큰 차감·환불 |
